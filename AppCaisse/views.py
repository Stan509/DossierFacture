...
def montant1(request):
    EntreeParDate = EntreeParDate.objects.all()
    return render(request, 'comptabilite.html', {'EntreeParDate': EntreeParDate})
...
def comptabilite(request):
    entreepardate_data = EntreeParDate.objects.all()
    montantpardate_data = MontantParDate.objects.all()
    cashparjour_data = CashParJour.objects.all()
    return render(request, 'comptabilite.html', {'entreepardate_data': entreepardate_data, 'montantpardate_data': montantpardate_data, 'cashparjour_data': cashparjour_data})
...
from .models import RetraitCaisse, MontantParDate
from django.db.models import Sum

def enregistrer_retrait(request):
    if request.method == 'POST':
        # Récupérez les données du formulaire de retrait
        montant = request.POST.get('montant')
        date = request.POST.get('date')

        # Enregistrez le retrait
        retrait = RetraitCaisse(montant=montant, date=date)
        retrait.save()

        # Mettez à jour MontantParDate en ajoutant le montant au total existant ou en créant un nouveau enregistrement si nécessaire
        montant_par_date, created = MontantParDate.objects.get_or_create(date=date)
        montant_par_date.montant_total = montant_par_date.montant_total + montant
        montant_par_date.save()

        # Redirigez l'utilisateur vers une page de confirmation ou ailleurs

    # Le reste de votre logique de vue
from .models import EntreeParDate

def entreepardate_view(request):
    entreepardate_data = EntreeParDate.objects.all()
    return render(request, 'comptabilite.html', {'entreepardate_data': entreepardate_data})

from .models import CashParJour

def cashparjour_view(request):
    cashparjour_data = CashParJour.objects.all()
    return render(request, 'comptabilite.html', {'cashparjour_data': cashparjour_data})
